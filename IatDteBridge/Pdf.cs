...
namespace IatDteBridge
{
    class Pdf
    {
        ...
        public Document OpenPdf(string dd, Documento doc, string fileName, string tipoCopia)//OpenPdf(Documento doc, String dd)
        {
           ...
           // Document pdf = new Document(PageSize.LETTER);
            ...
            iTextSharp.text.Image logo = iTextSharp.text.Image.GetInstance(@""+Microsoft.Win32.Registry.GetValue(@"HKEY_CURRENT_USER\Iat", "unidadIat", null).ToString()+":/IatFiles/config/logo.jpg");//
            //Marca de agua
            if (doc.RUTEmisor == "99505200-8")
              {
              Image fondoAgua;
              fondoAgua = Image.GetInstance(@""+Microsoft.Win32.Registry.GetValue(@"HKEY_CURRENT_USER\Iat", "unidadIat", null).ToString()+":/IatFiles/config/marcaagua.jpg");
              fondoAgua.Alignment = iTextSharp.text.Image.ALIGN_LEFT;
              fondoAgua.SetAbsolutePosition(190f, 450f); //
              pdf.Add(fondoAgua);
              }

            if (doc.RUTEmisor == "99505200-8")
            {
                logo.ScaleAbsolute(150f, 60f); // TODO valores para Asemaq 
            }
            else
            {
            ...
            float[] anchosCabecera;
            //  
            if (doc.RUTEmisor == "99505200-8")
            {
                anchosCabecera = new float[] { 270f, 430f, 300f }; // TODO Valores Asemaq
            }
            else
            {
                anchosCabecera = new float[] { 200f, 500f, 300f };   
            }
            ...
            if (doc.TipoDTE == 52 && doc.RUTEmisor != "99505200-8")
            {
 ...
 // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Cuarta fila +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            else
            {
                ...
                if (doc.TipoDTE == 33 && new ConnectDb().GetOC() == "True")
                {
            ...
            if (plu == "False")
            {
                detalle.ResetColumnCount(6);
                detalle.SetWidths(new float[] { 15f, 200f, 30f, 50f, 30f, 50f });
            }
            else
            {
            ...
            foreach (string j in headerDetalle)
            {
                ...
                if (plu == "False")
                {
                    if (j != "Codigo" && j != "Unidad") detalle.AddCell(celda);
                }
                else
                {
            ...
            foreach (var det in doc.detalle)
            {
                ...
                else
                {
                    ...
                    {
                        datosDetalle[puntero] = det.PrcItem.ToString("N"+cantDecimales, CultureInfo.CreateSpecificCulture("es-ES"));
                    }
                    else
                    {
                            ...
                            else
                            {
                                ...
                                switch (Convert.ToInt32(b.TpoDocRef))
                                {
                                    ...
                                    case 35: tipoDocRef = "BOLETA";
                                        break;
                                    case 801: tipoDocRef = "ORDEN DE COMPRA";
                                        break;
                                    case 46: tipoDocRef = "FACTURA DE COMPRA ELECTRÓNICA";
                                        break;
            ...
            string tipoDeTraslado = "Tipo Traslado: (" + doc.IndTraslado + ") " + nombreTraslado;
            if (doc.TipoDTE != 52) tipoDeTraslado = "";
            ...
            if (doc.CondEntrega == "True")
            {
                ...
                {
                    PdfPCell celdaCondEntrega = new PdfPCell(new Paragraph(tipoDeTraslado + "\n\n\r" + "Cita N°: " + doc.NroCita + " \nSello(s): " + doc.Sello + "\n\n\n", fuenteBold));
                ...
                else
                {
                    ...
                    if (doc.RUTEmisor == "99505200-8")
                    {
                        PdfPCell celdaCondEntrega = new PdfPCell(new Paragraph("O.T: " + doc.NroCita +"\n\r" 
                                                                              + "FORMA DE DESPACHO: " + doc.Sello +"\n\r"
                                                                              + "FORMA DE PAGO: "+ doc.formaPago + "\n\r"
                                                                              + tipoDeTraslado + "\n\r"
                                                                              + "CANCELADO:\n\r Fecha: ____/ ____/ ____/    Firma: ______________________\n\n\n"
                                                                              + "Se invita nuestros clientes a comunicar calquier sugerencia, reclamo o comentario "
                                                                              + "a nuestro mail: asemaq@asemaq.cl y visitar nuestra web www.asemaq.cl\n\r"
                                                                                  , fuenteBold7));
                        celdaCondEntrega.BorderWidth = 0;
                        celdaCondEntrega.HorizontalAlignment = 0;
                        tablaDTimbre.AddCell(celdaCondEntrega);
                    ...
                    else
                    {
                        ...
                        PdfPCell celdaCondEntrega = new PdfPCell(new Paragraph(tipoDeTraslado + "\n\n\r" + "Condición Entrega:  ___ Inmediato    ___ Retiro    ___ Despacho \n\n\n", fuenteBold));
                        celdaCondEntrega.BorderWidth = 0;
                        celdaCondEntrega.HorizontalAlignment = 0;
                        tablaDTimbre.AddCell(celdaCondEntrega);
                    }
        ...
        public void OpenPdfPrint(String dd, Documento doc, String fileName)
        {
            ...
            for( int copies = 0 ; copies<3 ;copies++)
            {
                ...
                if (copies == 2) 
                {
                    if (doc.TipoDTE == 33 || doc.TipoDTE == 34 || doc.TipoDTE == 46)
                    {
                        tipoCopia = "CEDIBLE";
                    }
                    if (doc.TipoDTE == 52)
                ...
                if (doc.RUTEmisor == "99505200-8")
                {
                    ...
                    fondoAgua = Image.GetInstance(@""+Microsoft.Win32.Registry.GetValue(@"HKEY_CURRENT_USER\Iat", "unidadIat", null).ToString()+":/IatFiles/config/marcaagua.jpg");
                    fondoAgua.Alignment = iTextSharp.text.Image.ALIGN_LEFT;
                    fondoAgua.SetAbsolutePosition(190f, 450f); // -> , desde abajo hacia arriba
                    pdf.Add(fondoAgua);
                }
                ...
                if (doc.CondEntrega == "True")
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // Agrega Condición de entrega del documento
                            PdfPCell celdaCondEntrega = new PdfPCell(new Paragraph(tipoDeTraslado + "\r\n\n" + "Condición Entrega:  ___ Inmediato    ___ Retiro    ___ Despacho \n\n\n", fuenteBold));
                            celdaCondEntrega.BorderWidth = 0;
                            celdaCondEntrega.HorizontalAlignment = 0;
                            tablaDTimbre.AddCell(celdaCondEntrega);
                        }
